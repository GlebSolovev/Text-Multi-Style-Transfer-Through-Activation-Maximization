...
def get_bt_input(input, args):
    ...
    tgt_mask = tgt_mask & get_cuda(
        Variable(subsequent_mask(tensor_tgt.size(-1)).to('cuda:%s' % args.gpu).type_as(tgt_mask.data)), args.gpu)
...
def get_cuda(tensor, gpu):
    return tensor.to(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
