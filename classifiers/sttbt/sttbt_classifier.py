...
from classifiers.style_classifier import StyleClassifier


class STTBTClassifier(StyleClassifier, ABC):
    ...
    def __init__(self, batch_size: int = 64, max_text_length_in_tokens: int = 50, gpu: bool = False):
        ...
        model.cuda() if self.gpu else model.cpu()

        self.model = model
        self.model.eval()

    def transform_to_batches(self, texts: List[str]) -> List[StyleClassifier.Batch]:
        batches = []
        batch = []
    ...
    def classify(self, batch: StyleClassifier.Batch) -> List[float]:
    ...
    def __to_indexes_and_one_hot_encode(self, batch: StyleClassifier.Batch):
